...
try:
    ...
    partner_path = input("Enter the path for res_partner file.")
except SyntaxError:
	raise Exceptions('Put the path in quotes.')
else:
	os.chdir(str(the_dir))
	source_dir = os.getcwd() # path for the file to work on.
...
def make_new_file(working_directory, working_file, partner_path):
...
for item in items:
    make_new_file(source_dir, item, partner_path)